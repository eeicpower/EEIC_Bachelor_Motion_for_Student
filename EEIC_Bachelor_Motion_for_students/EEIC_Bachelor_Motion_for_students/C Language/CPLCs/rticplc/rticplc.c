...
{	
	...
	if (flag_exptype==2) {
		// 位置指令値
		// type1: step reference (angle ref [rad], start time [s], end time [s], current time [s], position reference [rad])
		ctrl_step_input_with_end(M_PI/2, 0, 5, timer, &pos_ref_rad); //make step reference
		// type2: sin wave input
		//pos_ref_rad = amp_sinref_rad * sin(2 * M_PI * timer);

		pos_error_rad = pos_ref_rad - motor_pos_rad;
		
		func_pd_awu(pos_error_rad, &torque_cmd_Nm, 5.0); //FB control output with anti windup (max = 5)
		counter++;
	}
	...
	if (flag_exptype==3) {
		...
		func_pd(pos_error_rad, &v0); // FB control output without anti windup
		...
		vdistest = -v2;
		func_LFmath(v1, &qout);
		func_INVQmath(motor_pos_rad, &invqout);
		// #endregion DOB
